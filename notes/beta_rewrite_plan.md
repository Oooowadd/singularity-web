...
- 三张 agent 卡（Clerk / Muse / Poet），每张显示累计产出 + 7 天 delta；该 agent 有正在跑的任务时卡片右上脉动小点
...
- 主页链接 + 对标频道列表
- **URL 验证 + 预览**：单链接「验证」按钮、对标列表「批量验证」按钮。YouTube 走官方 Data API 优先 + TikHub 兜底；XHS 直接拉粉丝 / 获赞 / 头像 / IP 归属地。让用户在 add 时就看到「这真的是我想要的人」而不是事后跑出空数据
...
- 三种 SOP（V4 Pro 生成，Markdown 渲染）：
  ...
  - **ai_reference**：AI 友好的结构化 SOP，给 Muse/Poet prompt 注入
...
- 选题卡：故事角度 / 数据 / 与本频道契合点 / 爆款触发因素；一键审批 = 立即翻状态（乐观更新，不等 router.refresh）
...
**Imagination gate**（防 LLM 编造）：分类阶段要求 transcript 必须真实存在。YouTube 视频 floor 200 字；XHS 图文 floor 50 字（title+desc 通常 80-300 字，已是真实创作内容）。
...
- **Trigger.dev**：Free plan 1h/task 硬限会撞墙。已升 Hobby（$10/mo, 7 天上限）；代码侧 `maxDuration: 14400` (4h) 全部任务统一
...
- **TikHub 速率**：1 req/sec per route（跨 route 才能并发）；YouTube `get_video_info` 必须用 `web_v2`，v3 返回 raw playerResponse 没 videoDetails
...
- **TikHub XHS `web_v3/fetch_note_detail`** 强制要 `xsec_token`，缺会 422；用 `get_note_info_v4` 或 `get_user_notes_v2` 不需要
...
每视频 1 quota。prefilter spam / 引战 + 多语言容忍。注入 SOP prompt 进一步抓 viral trigger。约 2-3h 投入。
...
外资实体不能直接备案，需 WFOE / JV / 中国合作方，端到端 $5-15K + 3-6 个月。WeChat web 登录可绕过 ICP（¥300 网站应用审核）；Mini Program 后端 callback 域名仍需 ICP。**起点越晚 WeChat 上线越晚**。
...
> 用户主要在中国大陆 + 港澳台。区域决策围绕「数据库延迟 / Trigger.dev cloud worker 出口 IP / 服务商合规」三个轴。
...
| Logto Cloud | 之后我们决定 | Auth 黑盒；不影响业务延迟（只在登录瞬间走一次） |
