...
3. **逐条理解** — 最多同时 4 条，每条依次：
   ...
   2. 拆解视频 — Pro，prompt [`buildVideoAnalysisPrompt`](../packages/prompts/src/clerk.ts#L58)。产出结构化拆解（开头钩子 / 框架 / 节奏 / 选题角度，带 [mm:ss] 时间戳引用）。
   3. 看封面 — Vision，与上一步并行。
...
5. **生成 3 份 SOP** — 三份并行，各一次 Pro 调用：
   - 真人版 [`buildHumanSopPrompt`](../packages/prompts/src/clerk.ts#L156) —— 创作者本人看的创作手册，跟随频道语言。
   - AI 参考版 [`buildAiSopReferencePrompt`](../packages/prompts/src/clerk.ts#L231) —— 给 Poet 写稿 AI 读的结构化参考，强制英文。
   - 爆款版 [`buildHottestSopPrompt`](../packages/prompts/src/clerk.ts#L329) —— 拆解最高播放视频的套路，跟随频道语言（需该视频有文字稿，否则跳过）。
...
之后同样 Pro 拆解（[`buildVideoAnalysisPrompt`](../packages/prompts/src/clerk.ts#L58)）+ Vision 封图分析（多图用图集分析、单图 / 视频封面用单图分析），最终同样汇总成 3 份 SOP（与 1.2 第 5–6 步一致）。
...
4. **相关性判断** — Flash，prompt [`buildClassificationPrompt`](../packages/prompts/src/muse.ts#L15)。判断这条视频有没有"可迁移的爆款机制"（看的是钩子 / 情绪 / 叙事结构能不能借，不是题材是否相同）。不相关或文字稿过短则到此为止。
5. **提炼爆款触发器** — Pro，prompt [`buildViralTriggerPrompt`](../packages/prompts/src/muse.ts#L61)。读完整文字稿，提炼"点击 / 观看 / 转发"三类触发点。
6. **生成选题** — Pro，prompt [`buildIdeaGenerationPrompt`](../packages/prompts/src/muse.ts#L101)。基于触发器为本频道生成 N 条选题（故事角度 / 事实数据 / 为何相似 / 封面概念 / 钩子类型 / 风险点）。
7. **存库** — 选题入库，前端展示供挑选。
...
2. **生成圣经** — Pro，prompt [`buildChannelBiblePrompt`](../packages/prompts/src/poet.ts#L9)。产出：TOPIC + 频道定位 / 信息源 / 选题框架 三节。
...
2. **拆解选题** — Pro，prompt [`buildTopicAnalysisPrompt`](../packages/prompts/src/poet.ts#L274)。结合参考 + 圣经 + AI 参考 SOP，产出：故事角度 / 事实数据 / 原文事实（保留原语言，数字专名不翻）/ 为何契合 / 爆款触发点。
...
1. **写稿** — Pro，prompt [`buildScriptWritingPrompt`](../packages/prompts/src/poet.ts#L84)（一次出全文）。
2. **品牌口头禅校验** — 若圣经里的标志性话术没出现在稿子里，补提示重写一次。
3. **口语化（仅中文）** — Pro，prompt [`buildChineseHumanizerPrompt`](../packages/prompts/src/poet.ts#L327)，改写成真人开口的口语。
4. **存库**。

长稿：

1. **列大纲** — Pro，prompt [`buildLongFormOutlinePrompt`](../packages/prompts/src/poet.ts#L166)。按比例分配各段字数（钩子 / 铺垫 / 正文 / CTA / 高潮 / 收尾），产出分段大纲。
2. **逐段扩写** — Pro，prompt [`buildSectionExpandPrompt`](../packages/prompts/src/poet.ts#L226)，按大纲一段一段写（每段一次调用），最后拼接。
...
| Prompt 函数 | 位置 | 作用 |
|---|---|---|
| [`buildVideoAnalysisPrompt`](../packages/prompts/src/clerk.ts#L58) | clerk.ts:58 | 单条视频 / 笔记：文字稿 + 元数据 → 结构化拆解 |
| [`buildHumanSopPrompt`](../packages/prompts/src/clerk.ts#L156) | clerk.ts:156 | 汇总拆解 → 创作者版 SOP（手册）|
| [`buildAiSopReferencePrompt`](../packages/prompts/src/clerk.ts#L231) | clerk.ts:231 | 汇总拆解 → AI 写稿参考 SOP（英文）|
| [`buildHottestSopPrompt`](../packages/prompts/src/clerk.ts#L329) | clerk.ts:329 | 最高播放视频 → 爆款深拆 SOP |
...
| [`buildViralTriggerPrompt`](../packages/prompts/src/muse.ts#L61) | muse.ts:61 | 相关视频 → 点击 / 观看 / 转发触发器 |
| [`buildIdeaGenerationPrompt`](../packages/prompts/src/muse.ts#L101) | muse.ts:101 | 触发器 → 本频道 N 条选题 |
| [`buildChannelBiblePrompt`](../packages/prompts/src/poet.ts#L9) | poet.ts:9 | 频道想法 → 频道圣经（基准文档）|
| [`buildTopicAnalysisPrompt`](../packages/prompts/src/poet.ts#L274) | poet.ts:274 | 选题 + 参考 → 结构化选题 |
| [`buildScriptWritingPrompt`](../packages/prompts/src/poet.ts#L84) | poet.ts:84 | 选题 + 圣经 + SOP → 短稿全文 |
| [`buildLongFormOutlinePrompt`](../packages/prompts/src/poet.ts#L166) | poet.ts:166 | 长稿：选题 → 分段大纲 |
| [`buildSectionExpandPrompt`](../packages/prompts/src/poet.ts#L226) | poet.ts:226 | 长稿：单段大纲 → 成段正文 |
| [`buildChineseHumanizerPrompt`](../packages/prompts/src/poet.ts#L327) | poet.ts:327 | 中文稿 → 真人口语化改写 |
