...
> **占位符**：`{{中文标签}}` 表示运行时填入的变量；数量类字段（视频数、目标字数、时长、播放量等）以示例数值展示。
...
- 模型：DeepSeek Pro
- 源码：[`buildVideoAnalysisPrompt`](../packages/shared/src/prompts/clerk.ts#L58)　·　流程：Clerk 1.2 · 步骤 3.2
...
- 模型：DeepSeek Pro
- 源码：[`buildHumanSopPrompt`](../packages/shared/src/prompts/clerk.ts#L156)　·　流程：Clerk 1.2 · 步骤 5
...
- 模型：DeepSeek Pro
- 源码：[`buildAiSopReferencePrompt`](../packages/shared/src/prompts/clerk.ts#L231)　·　流程：Clerk 1.2 · 步骤 5
...
- 模型：DeepSeek Pro
- 源码：[`buildHottestSopPrompt`](../packages/shared/src/prompts/clerk.ts#L329)　·　流程：Clerk 1.2 · 步骤 5
...
After the Parts, append a **Retention Tape** section: a single chronological list of every retention move (open loop, rehook, specificity spike, pattern break) with its `[m:ss]` and a 5-word description.
...
- 模型：DeepSeek Flash
- 源码：[`buildCommentsSummaryPrompt`](../packages/shared/src/prompts/clerk-comments.ts#L7)　·　流程：Clerk 1.2 · 步骤 4
...
- 源码：[`buildSeriesDetectPrompt`](../packages/shared/src/prompts/clerk-series.ts#L7)　·　流程：Clerk 1.4（独立按钮）
...
- 模型：DeepSeek Flash
- 源码：[`buildClassificationPrompt`](../packages/shared/src/prompts/muse.ts#L15)　·　流程：Muse 2.2 · 步骤 4
...
- 模型：DeepSeek Pro
- 源码：[`buildViralTriggerPrompt`](../packages/shared/src/prompts/muse.ts#L61)　·　流程：Muse 2.2 · 步骤 5
...
- 模型：DeepSeek Pro
- 源码：[`buildIdeaGenerationPrompt`](../packages/shared/src/prompts/muse.ts#L101)　·　流程：Muse 2.2 · 步骤 6
...
- 模型：DeepSeek Pro
- 源码：[`buildChannelBiblePrompt`](../packages/shared/src/prompts/poet.ts#L9)　·　流程：Poet 3.1 · 步骤 2
...
- 模型：DeepSeek Pro
- 源码：[`buildTopicAnalysisPrompt`](../packages/shared/src/prompts/poet.ts#L274)　·　流程：Poet 3.2 · 步骤 2
...
- 模型：DeepSeek Pro
- 源码：[`buildScriptWritingPrompt`](../packages/shared/src/prompts/poet.ts#L84)　·　流程：Poet 3.3 · 短稿步骤 1
...
Write a COMPLETE, ready-to-film script in **Chinese (中文)**. Target **1000 characters (字)** — this is not a suggestion, it is the required output length. Do not end the script before reaching at least 90% of this count (900 characters (字)). If you finish all sections early, expand the ITEM sections with more specific examples, detail, and emotional beats until you reach the minimum.
...
- 模型：DeepSeek Pro
- 源码：[`buildLongFormOutlinePrompt`](../packages/shared/src/prompts/poet.ts#L166)　·　流程：Poet 3.3 · 长稿步骤 1
...
- 模型：DeepSeek Pro
- 源码：[`buildSectionExpandPrompt`](../packages/shared/src/prompts/poet.ts#L226)　·　流程：Poet 3.3 · 长稿步骤 2
...
- 模型：DeepSeek Pro
- 源码：[`buildChineseHumanizerPrompt`](../packages/shared/src/prompts/poet.ts#L327)　·　流程：Poet 3.3 · 短/长稿口语化步骤
