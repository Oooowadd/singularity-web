...
- 启动后抓取最近 N 个视频（YouTube：YT Data API metadata + TikHub audio streams + Deepgram/Qwen ASR；XHS：TikHub `get_user_notes_v2` 一次拿全 + 视频 note 走 ASR、图文 note 用 title+desc）
